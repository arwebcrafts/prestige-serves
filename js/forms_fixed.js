...
function openDefendantModal(editIndex = -1) {
  ...
  if (editIndex > -1) {
    ...
    document.getElementById('def-state-input').value = def.stateLabel || '';
...
function clearDefendantForm() {
  ...
  inputs.forEach(input => {
    if(input.id !== 'def-country' && input.id !== 'def-edit-index' && input.id !== 'def-state-input' && input.id !== 'def-state-value') {
      if(input.id !== 'def-city' && input.id !== 'def-city-value') {
        input.value = '';
      }
    }
    document.getElementById('def-state-input').value = '';
    document.getElementById('def-state-value').value = 'CA';
}
...
function saveDefendant() {
  ...
  const defendantData = {
    ...
    stateLabel: document.getElementById('def-state-input').value,
