          ...
          <tr>
            <td style="background-color:#f8fafc;padding:30px 40px;border-top:1px solid #e2e8f0;">
              <p style="margin:0;font-size:13px;color:#94a3b8;text-align:center;line-height:1.6;">
                ...
                Phone: 424-235-3089 | Email: <a href="mailto:prestigervesllc@gmail.com" style="color:#94a3b8;">prestigervesllc@gmail.com</a>
              </p>
            </td>
          </tr>
        </table>
...
/**
 * Beautiful HTML email sent TO THE CLIENT as the AI auto-reply.
 * @param {{ clientName: string, replyText: string, serviceType: string }} data
 */
export function buildAiClientReplyHtml({ clientName, replyText, serviceType }) {
  // Convert plain-text reply (with newlines) to HTML paragraphs
  const replyHtml = (replyText || '')
    .split(/\n\n+/)
    .map(para => `<p style="margin:0 0 16px 0;font-size:15px;line-height:1.75;color:#444444;">${para.replace(/\n/g, '<br>')}</p>`)
    .join('');
...
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Re: Your Inquiry — Prestige Serves</title>
</head>
<body style="margin:0;padding:0;font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;background-color:#f0f4f8;">
  <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background-color:#f0f4f8;">
    <tr>
      <td align="center" style="padding:40px 20px;">
        <table role="presentation" width="600" cellspacing="0" cellpadding="0" style="background-color:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 8px 32px rgba(0,0,0,0.12);">

          <!-- Header -->
          <tr>
            <td style="background:linear-gradient(135deg,#1a3a5c 0%,#2c5282 50%,#1a3a5c 100%);padding:44px 40px 36px 40px;text-align:center;">
              <h1 style="margin:0 0 6px 0;font-size:26px;font-weight:800;color:#ffffff;letter-spacing:2px;text-transform:uppercase;">PRESTIGE SERVES</h1>
              <p style="margin:0;font-size:13px;color:rgba(255,255,255,0.75);letter-spacing:3px;text-transform:uppercase;">Professional Process Serving</p>
            </td>
          </tr>
          ...
          <tr>
            <td style="padding:28px 40px 0 40px;text-align:center;">
              <span style="display:inline-block;background:linear-gradient(135deg,#e8f5e9,#c8e6c9);color:#1b5e20;font-size:12px;font-weight:700;padding:8px 22px;border-radius:20px;text-transform:uppercase;letter-spacing:1.5px;">✉️ Response to Your Inquiry${serviceType ? ` — ${serviceType}` : ''}</span>
            </td>
          </tr>
          ...
          <tr>
            <td style="padding:28px 40px;text-align:center;">
              <a href="https://prestigeserves.com/request.html"
                 style="display:inline-block;background:linear-gradient(135deg,#1a3a5c,#2c5282);color:#ffffff;font-size:15px;font-weight:700;padding:16px 42px;border-radius:10px;text-decoration:none;letter-spacing:0.5px;box-shadow:0 4px 14px rgba(26,58,92,0.35);">
                🗂️ Submit a Service Request
              </a>
              <p style="margin:16px 0 0 0;font-size:13px;color:#94a3b8;">Or reply to this email and we'll get right back to you.</p>
            </td>
          </tr>
              ...
              <table role="presentation" width="100%" cellspacing="0" cellpadding="0">
                <tr>
                  <td style="text-align:center;">
                    <p style="margin:0 0 6px 0;font-size:14px;font-weight:700;color:#1a3a5c;">Prestige Serves LLC</p>
                    <p style="margin:0;font-size:12px;color:#94a3b8;line-height:1.8;">
                      1240 S Corning Street, Los Angeles, CA 90035<br>
                      📞 <a href="tel:4242353089" style="color:#94a3b8;text-decoration:none;">424-235-3089</a> &nbsp;|&nbsp;
                      ✉️ <a href="mailto:info@prestigeserves.com" style="color:#94a3b8;text-decoration:none;">info@prestigeserves.com</a>
                    </p>
                    <p style="margin:12px 0 0 0;font-size:11px;color:#cbd5e1;">This reply was generated by our AI assistant. A team member will follow up to confirm details.</p>
                  </td>
                </tr>
              </table>
            </td>
...
/**
 * Alert email sent TO THE OWNER after AI auto-replies.
 * Shows the original inquiry, the AI's reply, and a one-click [Take Over] button.
 * @param {{ clientName, clientEmail, serviceType, inquiry, aiReplyText, handoffUrl, attachmentCount }} data
 */
export function buildOwnerAiAlertHtml({ clientName, clientEmail, serviceType, inquiry, aiReplyText, handoffUrl, attachmentCount = 0 }) {
  const inquiryHtml = (inquiry || 'No message body').replace(/\n/g, '<br>');
  const replyHtml = (aiReplyText || '').replace(/\n/g, '<br>');
...
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>AI Auto-Replied — Prestige Serves</title>
</head>
<body style="margin:0;padding:0;font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;background-color:#f0f4f8;">
  <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background-color:#f0f4f8;">
    <tr>
      <td align="center" style="padding:40px 20px;">
        <table role="presentation" width="620" cellspacing="0" cellpadding="0" style="background-color:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 8px 32px rgba(0,0,0,0.12);">

          <!-- Header -->
          <tr>
            <td style="background:linear-gradient(135deg,#1a3a5c 0%,#2c5282 100%);padding:36px 40px;text-align:center;">
              <h1 style="margin:0 0 6px 0;font-size:22px;font-weight:800;color:#ffffff;letter-spacing:1px;">🤖 AI Auto-Reply Sent</h1>
              <p style="margin:0;font-size:13px;color:rgba(255,255,255,0.8);">Prestige Serves — Owner Notification</p>
            </td>
          </tr>
          ...
          <tr>
            <td style="padding:28px 40px 0 40px;">
              <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background:#fffbeb;border:1px solid #fcd34d;border-radius:10px;">
                <tr>
                  <td style="padding:18px 24px;">
                    <p style="margin:0;font-size:14px;color:#92400e;line-height:1.6;">
                      ⚡ <strong>The AI assistant automatically replied to this inquiry.</strong><br>
                      If you want to personally handle this client, click the button below to silence the AI.
                    </p>
                  </td>
                </tr>
              </table>
          ...
          <tr>
            <td style="padding:24px 40px 0 40px;">
              <p style="margin:0 0 10px 0;font-size:11px;font-weight:700;color:#94a3b8;text-transform:uppercase;letter-spacing:1.5px;">Client Details</p>
              <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;">
                <tr>
                  <td style="padding:14px 20px;border-bottom:1px solid #e2e8f0;">
                    <span style="font-size:11px;color:#94a3b8;text-transform:uppercase;">Name</span>
                    <p style="margin:4px 0 0 0;font-size:15px;color:#1a3a5c;font-weight:600;">${clientName}</p>
                  </td>
                  <td style="padding:14px 20px;border-bottom:1px solid #e2e8f0;">
                    <span style="font-size:11px;color:#94a3b8;text-transform:uppercase;">Email</span>
                    <p style="margin:4px 0 0 0;font-size:15px;"><a href="mailto:${clientEmail}" style="color:#2563eb;text-decoration:none;">${clientEmail}</a></p>
                  </td>
                </tr>
                <tr>
                  <td style="padding:14px 20px;" colspan="2">
                    <span style="font-size:11px;color:#94a3b8;text-transform:uppercase;">Service Requested</span>
                    <p style="margin:4px 0 0 0;font-size:15px;color:#333;font-weight:600;">${serviceType || 'General Inquiry'}${attachmentCount > 0 ? ` &nbsp;·&nbsp; 📎 ${attachmentCount} attachment${attachmentCount > 1 ? 's' : ''}` : ''}</p>
                  </td>
                </tr>
              </table>
            </td>
          ...
          <tr>
            <td style="padding:20px 40px 0 40px;">
              <p style="margin:0 0 10px 0;font-size:11px;font-weight:700;color:#94a3b8;text-transform:uppercase;letter-spacing:1.5px;">📩 Client's Inquiry</p>
              <div style="background:#f8fafc;border-left:4px solid #2563eb;border-radius:0 8px 8px 0;padding:16px 20px;font-size:14px;line-height:1.7;color:#555;">
                ${inquiryHtml}
              </div>
            </td>
          </tr>
          ...
          <tr>
            <td style="padding:20px 40px 0 40px;">
              <p style="margin:0 0 10px 0;font-size:11px;font-weight:700;color:#94a3b8;text-transform:uppercase;letter-spacing:1.5px;">🤖 AI Reply (Sent to Client)</p>
              <div style="background:#f0fdf4;border-left:4px solid #16a34a;border-radius:0 8px 8px 0;padding:16px 20px;font-size:14px;line-height:1.7;color:#333;">
                ${replyHtml}
              </div>
            </td>
          </tr>
          ...
          <tr>
            <td style="padding:32px 40px;text-align:center;">
              <p style="margin:0 0 16px 0;font-size:14px;color:#555;">Want to personally handle this client from now on?</p>
              <a href="${handoffUrl}"
                 style="display:inline-block;background:linear-gradient(135deg,#dc2626,#b91c1c);color:#ffffff;font-size:15px;font-weight:700;padding:16px 40px;border-radius:10px;text-decoration:none;letter-spacing:0.5px;box-shadow:0 4px 14px rgba(220,38,38,0.4);">
                🤝 Take Over This Thread
              </a>
              <p style="margin:14px 0 0 0;font-size:12px;color:#94a3b8;">Clicking this button will silence the AI for this inquiry permanently.</p>
            </td>
          </tr>
          ...
          <tr>
            <td style="background-color:#f8fafc;padding:24px 40px;border-top:1px solid #e2e8f0;text-align:center;">
              <p style="margin:0;font-size:12px;color:#94a3b8;line-height:1.8;">
                <strong style="color:#1a3a5c;">Prestige Serves LLC</strong> · AI Assistant System<br>
                1240 S Corning Street, Los Angeles, CA 90035
              </p>
            </td>
          </tr>
