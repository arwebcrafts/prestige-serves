...
async function ensureInvoicesTable(sql) {
  await sql`
    ...
    ALTER TABLE invoices ADD COLUMN IF NOT EXISTS stripe_pay_url TEXT;
  `;
}
...
function sanitizeInvoicePayload(body) {
  ...
  return {
    ...
    stripe_pay_url: body.stripe_pay_url ? String(body.stripe_pay_url).trim().slice(0, 500) : 'https://buy.stripe.com/4gM4gzg8xdrR0Uxfrf6sw0n',
  };
}
...
function publicInvoiceView(invoice) {
  return {
    ...
    stripe_pay_url: invoice.stripe_pay_url || 'https://buy.stripe.com/4gM4gzg8xdrR0Uxfrf6sw0n',
  };
}
